...
The relations between the Minkowski actions written before and the Euclidean actions are:
\begin{align*}
    S[\phi] =& \i S^E[\phi] = \i \dV \pr{\frac12\partial^\mu\phi\partial_\mu\phi+\frac12m^2\phi^2+V(\phi)}
\end{align*}
where the superscript $E$ means that the action is written in the Euclidean spacetime.

\section{Lattice Field Theory}
This section is based on standard quantum field theory textbooks cited before and lattice field theory textbooks \cite{Montvay:1994cy, Gattringer:2010zz, DeGrand:2006zz}.
