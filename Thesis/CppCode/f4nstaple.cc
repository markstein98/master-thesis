void f4nstaple(dc *stot, int site, int mu, int first_direction, int use_smeared) {

// Computes the triangular staples from (site) to (site+a mu) in the negative 
  ...
  for(j=0; j<nStaples; j+=2){
    ...
    if(dir2>0){
      ...
      if(nextSite!=neighbor_plus[dir2*nsites+vertex]){
        fprintf(stderr, "[ERROR]: nStaple does not close: site=%d, mu=%d, dir1=", site, mu);
