...
For this reason, Wilson's idea was to put the gauge group (and not algebra) elements, namely $U_\mu(x) = e^{\i a g A_\mu(x)}$.\\
...
In the following part it is shown that in the naive continuum limit ($a\to0$) the Wilson action reproduces the Yang-Mills action of \eqref{1:YMActionEuclidean}.
...
\begin{align*}
    ...
    =& \frac{g^2\beta}{8N} \underbrace{a^4\sum_{x,\mu,\nu}}_{\to\dV}\Fmunu^i F^{i\mu\nu} \numthis \label{1:WilsonSmalla}
\end{align*}
...
As mentioned below \eqref{1:GaugeTransformLinkVariable}, the trace of the product of link variables along any closed path is a gauge-invariant quantity.
...
%TODO: add image of Wilson loop
Wilson loops are important for a variety of reasons, that will not be elaborated further as this is not the focus of this project, however it is worth knowing that they can be used as operators for purely gluonic bound states, the so-called \emph{glueballs}: the mass spectrum of these states can be obtained through the exponential decay of their correlation functions.
...
Thanks to periodicity, a whole new class of closed paths arises: all the paths that wind around the time direction.
...
\end{equation}
The Polyakov loop has a lot of interesting properties: its physical interpretation is the world-line of a single static quark (\ie its mass is infinite), therefore its expectation value is an order parameter of the deconfinement transition.
