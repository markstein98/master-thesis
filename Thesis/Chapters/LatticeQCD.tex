...
%TODO: check bibliography and add coxeter-regular polytopes
\subsection{Lattice}
...
\end{figure}\\
In $D=4$, the SH lattice has the fundamental region (the smallest $D$-dimensional polyhedron) that is a $4$-dimensional hypercube, also called a tesseract. This means that a SH lattice can be seen as a tassellation of the spacetime with tesseracts as elementary cells.
...
The plaquette (the simplest bidimensional figure) is a square. This will be important when implementing gauge theories on the lattice.
