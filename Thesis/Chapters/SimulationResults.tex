...
Each site in the SH lattice has only $8$ nearest neighbours, individuated by $4$ different directions, the vectors $\pr{1,0,0,0}$, $\pr{0,1,0,0}$, $\pr{0,0,1,0}$, $\pr{0,0,0,1}$ (direction $3$), and their opposites.\\
...
As explained in \secref{Sec3:BCTlatticeGaugeTheories}, using action \eqref{3:BCTAction}, the plaquettes are triangular, therefore the staples are computed in a different way than the case of the SH lattice.\\
...
In lines \texttt{743} to \texttt{1063} of \coderef{A1:initialize743}{initialize.cc} this array is initialized in the following way: for each direction, $4$ couples of directions are given.
...
Direction $0$ is indicated as $24$ in order to distinguish between $\pr{+1,0,0,0}$ and $\pr{-1,0,0,0}$, since $+0=-0$.\\
Each of the four couples is such that the vector sum of the two directions forming the staple is equal to the vector representing the direction on which the considered link is defined.
...
In \secref{Sec3:BCTlatticeGaugeTheories} is stated that ``each edge is shared between $8$ triangles'', meaning that for each direction there must be $8$ different staples.
The $4$ missing are obtained from the ones already written, the \emph{positive} staples, by exchanging the first direction with the second (of each couple) and will be referred to as \emph{negative} staples\footnote{This is a reminiscence of the old version of the code, where \emph{positive} and \emph{negative} staples were not independent from each other, therefore were computed in two different routines in order to avoid overcounting when evaluating the plaquettes. This is not necessary anymore on the BCT lattice, as \emph{positive} and \emph{negative} staples are not counted twice.}.\\
