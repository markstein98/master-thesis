...
The mean value of the last $1000$ values of each dataset has also been computed, in order to study the behaviour of the average plaquette with respect to $\beta$.
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{PlaquetteBetaBCT.png}
    \caption{Mean value of the plaquette as a function of $\beta$. The purple data is from the smaller lattice ($n_t=4$, $n_s=8$), the green one is from the bigger lattice ($n_t=6$, $n_s=16$).}
\end{figure}
